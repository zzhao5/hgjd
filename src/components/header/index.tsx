  ...
  return (
    <header className={_s.wrap}>
      <div className={_s.main}>
        ...
        <div className={c(_s.nav, showNav ? _s.active : null)} onClick={(event) => event.stopPropagation()}>
          {
            menu.map(({ titles, mlist, urls, id }, index) => 
              <NavLink key={id} className={({isActive}) => c(_s.item, isActive ? _s.active : null)} to={urls} end={urls === '/'} 
              ...
              onMouseLeave={() => setShowSubNav((old) => { const t = {...old}; t[index] = false; return t;})}>
                  <span>
                    {titles}
                    {
                      mlist.length > 0 && <div className={c(_s.sub, showSubNav[index] ? _s.active : null)}>
                        {
                          mlist.map((subItem) => 
